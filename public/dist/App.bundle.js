...
function autocomplete(input, latInput, lngInput) {
  ...
  // if someone hits the enter in address field , dont submit the form
  ...
  axios.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n            <div class="popup">\n              <a href="/store/' + this.place.slug + '">\n               <img src="/uploads/' + (this.place.photo || 'store.png') + '" alt="' + this.place.name + '" />\n               <p>' + this.plae.name + ' - ' + this.place.location.address + '</p>\n             </a>\n           </div>\n          ';
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No resuts for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    // if they aren't pressing up, down and enter, who cares!
    ...
    var items = search.querySelectorAll('.search__resut');
    var next = void 0;
